...
def load_img(file_name1,file_name2,label,ext_h=820,ext_w=890):
    ...
    # edge_map1=cv.Canny(img1.numpy(),50,150)
    # edge_map1=np.expand_dims(edge_map1,axis=2)
    # edge_map1=tf.concat([edge_map1,img1],axis=-1)

    # inverse_map1=cv.bitwise_not(img1.numpy())
    # inverse_map1=np.expand_dims(inverse_map1,axis=2)
    # inverse_map1=tf.concat([inverse_map1,img1],axis=-1)

    rect_k=cv.getStructuringElement(cv.MORPH_RECT,(3,3))
    morph_map1=cv.morphologyEx(img1.numpy(),cv.MORPH_OPEN,rect_k)
    ...
    # kp, des = surf.detectAndCompute(img2.numpy(),None)
    # temp_img=np.squeeze(img2.numpy())
    # pt=[i.pt for i in kp]
    # pt=np.array(pt)
    # loc=np.zeros((pt.shape[0],4))
    # loc[:,0]=pt[:,1]-2
